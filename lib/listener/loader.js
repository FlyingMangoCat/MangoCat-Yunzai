...
class ListenerLoader {
  ...
  async loadEvent(file) {
    ...
    console.log("DEBUG loadEvent:", file)
    try {
      ...
      for (const type of Array.isArray(listener.event) ? listener.event : [listener.event]) {
        ...
        const eventName = listener.prefix + type
        const fn = Bot[on]
        console.log("DEBUG Bot.on type:", typeof fn, "isEventEmitter:", fn === (await import("events")).EventEmitter.prototype.on)
        Bot[on](eventName, listener[e].bind(listener))
        console.log("DEBUG after on, _events keys:", Object.keys(Bot._events || {}))
      }
    } catch (err) {
