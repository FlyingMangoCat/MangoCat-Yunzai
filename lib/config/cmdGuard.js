...
function isSilentReadCmd(cmd) {
  ...
  // 末尾确保没有更多命令（防 cd dir && git status && rm -rf data 绕过）
  if (/^cd\s+\S+\s*&&\s*(?:git|GIT)\s+(?:-C\s+["']?[^\s"']+["']?\s+)?(?:status|diff|log|rev-parse|show|branch|remote|tag|describe|ls-files|ls-tree|name-rev|rev-list|config|symbolic-ref|shortlog)\b[^;&|<>]*$/i.test(text)) return true;
