...
function isSilentUpdateCmd(cmd) {
  ...
  // 更新白名单仅限整条纯更新命令：含未引用组合符号（&&/管道/分号/重定向/命令替换）一律不静默，
  // 防 git pull && <任意命令> 借更新白名单前缀静默执行绕过风险提示
  ...
  if (/^(?:git|GIT)\s+(?:-C\s+["']?[^\s"']+["']?\s+)?(?:add|commit|stash|checkout)\b/i.test(text)) return true;
...
function isSilentReadCmd(cmd) {
  ...
  // 末尾确保没有更多命令，且不含 $/反引号/括号（防 cd dir && git log --pretty="$(xx)" 命令替换绕过）
  if (/^cd\s+\S+\s*&&\s*(?:git|GIT)\s+(?:-C\s+["']?[^\s"']+["']?\s+)?(?:status|diff|log|rev-parse|show|branch|remote|tag|describe|ls-files|ls-tree|name-rev|rev-list|config|symbolic-ref|shortlog)\b[^;&|<>$`()]*$/i.test(checkText)) return true;
...
function notifyPluginCmd(cmd, method, envPath) {
  ...
  // 含组合符号（&&/管道/分号/重定向/命令替换）仍不静默，防 command -v git && rm -rf data 绕过
  if (/^(?:command\s+-v\b|which\b|type\b)/i.test(cmd) && !/[|;&<>\$\(`]/.test(String(cmd))) return;
