...
function checkPath(p) {
  ...
  // glob 通配符目标（dat* / ./* / con?fig / confi[g]）：精确匹配不到核心路径，
  // 但 shell 展开后可能命中。改查基础目录（通配符前的目录部分）：
  //  - 基础目录为空或 "."（dat*、./*，展开范围为 cwd）→ 拦截
  //  - 基础目录为核心路径（config/*）→ 拦截
  //  - 基础目录无害（temp/*.log 合法清理）→ 放行，不误伤
  if (/[*?[]/.test(p)) {
    const meta = p.search(/[*?[]/);
    const base = p.slice(0, meta);
...
function checkCmdString(cmd) {
  ...
  // 远程脚本管道执行：curl/wget 管道给任意代码解释器（sh/bash/zsh/ksh/dash、python、perl、node、ruby、php、lua）
  // 不能只认 sh——curl|python3、curl|node 等换解释器形态同样远程代码执行
  if (/\b(?:curl|wget)\b[^|;<>]*\|[^;]*\b(?:(?:ba|z|k)?sh|dash|python3?|perl|node|ruby|php|lua)\b/i.test(text)) return "远程脚本管道执行(curl|解释器)";
  // 远程脚本进程替换：bash <(curl ...)
  if (/\b(?:ba)?sh\b[^<]*<\s*\(\s*(?:curl|wget)\b/i.test(text)) return "远程脚本管道执行(sh <(curl))";
  // 删除命令：rm/rmdir/del/rd 后跟危险路径
  // 全参数扫描：只查第一个目标会漏检 rm backup config（先无害后核心）与引号含空格路径 "config dir"
  const rmRe = /\b(?:rm|rmdir|del|rd)\b([^\n;|&]*)/gi;
  let rmM;
  while ((rmM = rmRe.exec(text)) !== null) {
    // 提取该段内全部目标 token（引号内路径取整体），跳过 -r/-f 选项与 Windows del 的 /q /s 类开关
    // 开关仅认单字母（/q、/s），防误跳 /root 等 Unix 单段绝对路径
    const tokenRe = /"([^"]*)"|'([^']*)'|(\S+)/g;
    let tk;
    while ((tk = tokenRe.exec(rmM[1])) !== null) {
      const tok = tk[1] !== undefined ? tk[1] : tk[2] !== undefined ? tk[2] : tk[3];
      if (/^-[a-zA-Z]+$/.test(tok) || /^\/[a-zA-Z]$/.test(tok)) continue;
      if (checkPath(tok)) return `删除命令目标【${tok}】`;
    }
  }
  return null;
}
...
function parseLocalScript(cmd) {
  ...
  // 归一化后再校验前缀：防 plugins/../lib/evil.sh 借 plugins/ 前缀骗过检查、
  // 实际执行 plugins 外的脚本（../ 消解后前缀即暴露）；归一化逃出项目（../ 开头）一律不静默
  const normalized = path.posix.normalize(scriptPath).replace(/\\/g, "/");
  if (normalized.startsWith("..") || path.isAbsolute(normalized)) return null;
  // 仅识别项目内 plugins/ 下的脚本；外部/绝对路径脚本不在静默范围（仍提示）
  if (!normalized.startsWith("plugins/")) return null;
  return normalized;
}
...
function isSilentUpdateCmd(cmd) {
  ...
  // 先剥除引号内字面量再检查组合符号（与 isSilentReadCmd 同规则）：
  // git commit -m "fix: a||b" 引号内 || 是字面量不算组合；双引号内含 $/反引号保留（可能命令替换）
  let checkText = text.replace(/"([^"]*)"/g, (all, inner) => (/[$`]/.test(inner) ? all : '""'));
  checkText = checkText.replace(/'([^']*)'/g, "''");
  // 更新白名单仅限整条纯更新命令：含未引用组合符号（&&/管道/分号/重定向/命令替换/换行分隔符）一律不静默，
  // 防 git pull && <任意命令>、git pull\n<任意命令>（换行也是 shell 命令分隔符）借更新白名单前缀静默执行绕过风险提示
  if (/[|;&<>\$\(`\n\r]/.test(checkText)) return false;
  // 拒绝小写 -c 配置注入（git 的配置选项是小写 -c，可让 pull/fetch 经 ext:: 传输执行任意命令，如
  // git -c protocol.ext.allow=always pull ext::sh -c xxx；大小写敏感检查，不误伤 -C 目录选项）
  if (/(?:^|\s)-c\s/.test(checkText)) return false;
  // 拒绝远程执行选项（--upload-pack/--receive-pack/--exec 可指定任意命令，借 fetch/push 静默执行）
  if (/\s--(?:upload-pack|receive-pack|exec)(?:\s|=|$)/.test(checkText)) return false;
  // 拒绝传输助手协议（pull/fetch/merge/push/submodule 的仓库参数可为 ext::sh -c xxx / fd:: 直接执行任意命令）
  if (/^(?:git|GIT)\s+(?:-C\s+["']?[^\s"']+["']?\s+)?(?:pull|fetch|merge|push|submodule)\b/i.test(text) && /[a-zA-Z0-9+.-]+::/.test(checkText)) return false;
  // exec 字符串模式：git pull / git fetch / git merge / git submodule update / git reset --hard 更新场景
  if (/^(?:git|GIT)\s+(?:-C\s+["']?[^\s"']+["']?\s+)?(?:pull|fetch|merge|submodule\s+update|reset\s+--hard(?:\s+origin(?:\/\S+)?)?)/i.test(text)) return true;
  // git 版本控制写操作：add / commit / stash / checkout 等（本体 update.js preCommit 机制依赖，无害）
  // stash 拒绝 clear/drop（清空/丢弃存档）；checkout 拒绝 -- 与裸 .（丢弃本地未提交改动）
  if (/^(?:git|GIT)\s+(?:-C\s+["']?[^\s"']+["']?\s+)?(?:add|commit|stash(?!\s+(?:clear|drop)\b)|checkout(?!\s+--)(?!\s+\.\s*$))\b/i.test(text)) return true;
  ...
  // 进程管理：pnpm restart / npm run restart / yarn run start 等（更新后重启、启停场景）
  if (/^(?:pnpm|npm|yarn)(?:\s+-[a-zA-Z]+\s*)*\s+(?:run\s+)?(?:restart|start|stop|kill|pm2)\b/i.test(text)) return true;
...
function isSilentReadCmd(cmd) {
  const text = String(cmd || "").trim();
  if (!text) return false;
  // 先剥除引号内字面量再检查组合符号：--pretty=format:"%h||%s" 这类引号内的 || 是普通字符不是管道
  // 双引号内含 $ 或反引号的保留（可能构成命令替换，仍按含组合符号处理）；单引号内 shell 永不展开，直接剥除
  let checkText = text.replace(/"([^"]*)"/g, (all, inner) => (/[$`]/.test(inner) ? all : '""'));
  checkText = checkText.replace(/'([^']*)'/g, "''");
  // cd dir && git 只读查询（切换目录后查询，update.js 等场景，无害）
  // 末尾确保没有更多命令，且不含 $/反引号/括号/换行（防 cd dir && git log --pretty="$(xx)" 命令替换绕过）
  if (/^cd\s+\S+\s*&&\s*(?:git|GIT)\s+(?:-C\s+["']?[^\s"']+["']?\s+)?(?:status|diff|log|rev-parse|show|branch|remote|tag|describe|ls-files|ls-tree|name-rev|rev-list|config\s+(?:--get(?:-all|-regexp)?|--list|-l)|symbolic-ref|shortlog)\b[^;&|<>$`()\n\r]*$/i.test(checkText)) return true;
  // 含组合符号（管道/分号/重定向/&&/命令替换/进程替换/换行分隔符）一律不静默
  if (/[|;&<>\$\(`\n\r]/.test(checkText)) return false;
  // git 只读查询命令（status/diff/log/rev-parse/show/branch/remote/ls-* 等）
  // 注意排除 reset/checkout/clean/rm 等有副作用的子命令
  // config 仅限 --get/--list 等纯读形式：git config <key> <value> 是写操作（可改 hooksPath 等），不静默
  if (/^(?:git|GIT)\s+(?:-C\s+["']?[^\s"']+["']?\s+)?(?:status|diff|log|rev-parse|show|branch|remote|tag|describe|ls-files|ls-tree|name-rev|rev-list|config\s+(?:--get(?:-all|-regexp)?|--list|-l)|symbolic-ref|shortlog)\b/i.test(text)) return true;
  // fastfetch/neofetch 系统信息采集
  if (/^(?:fastfetch|neofetch)\b/i.test(text)) return true;
  // chromium/chrome 浏览器渲染（liulian-plugin 等渲染页面用），仅系统路径可信（isTrustedBinary 校验）
  // 用 path.basename 取命令名（支持 /usr/bin/chromium 绝对路径与裸命令名）
  const m = text.match(/^"?([^\s"'|;&<>]+)"?/);
  const bin = m ? path.basename(m[1]) : "";
  if (/^(?:chromium|chromium-browser|google-chrome|chrome|headless_shell)$/i.test(bin)) return true;
  return false;
}
 ...
 * 校验静默命令的可执行文件是否可信（防伪装攻击）
 * 攻击者可能在项目内（plugins/ 等）放置与 git/fastfetch 同名的恶意脚本，
 * 或让插件用相对路径/项目内路径执行，或通过自定义 env.PATH 劫持命令解析。
 * 静默前定位真实二进制路径（优先使用实际执行环境的 PATH，防 PATH 劫持）：
 * - 命令 token 为相对路径（./x、plugins/x）→ 不静默（脚本分支由内容检测覆盖）
 * - 命令 token 为绝对路径 → 解析后校验是否位于项目内
 * - 裸命令名 → 用实际执行环境 PATH 定位（envPath 优先，默认进程 PATH）
 * - 自定义 env.PATH 含项目内目录 → 判定为 PATH 劫持，不静默
 * - 定位失败 → 保守不静默
 * 仅系统路径（项目外）的真实二进制才可静默
 */
function isTrustedBinary(cmd, envPath) {
  let text = String(cmd || "").trim();
  // 组合命令（cd dir && git log）：可信校验针对 && 后实际执行的命令——
  // cd 只是切换目录无二进制，且 command -v cd 返回内建名，解析成项目内路径会误判不可信
  const cdM = text.match(/^cd\s+["']?[^\s"']+["']?\s*&&\s*(.+)$/i);
  if (cdM) text = cdM[1].trim();
  const m = text.match(/^"?([^\s"'|;&<>]+)"?/);
  if (!m) return false;
  const bin = m[1];
  // 相对路径（./x、plugins/x、../x）不静默：脚本分支由 parseLocalScript 内容检测覆盖
  if (bin.startsWith(".") || bin.startsWith("plugins/") || bin.startsWith("..")) return false;
  // 绝对路径（/usr/bin/chromium、C:\xx）：不依赖 PATH 查找，解析后校验是否位于项目内
  ...
  // 裸命令名：先查 PATH 劫持（自定义 PATH 含项目内目录 → 不静默），再用实际执行环境的 PATH 定位
  if (envPath && envPath !== process.env.PATH) {
    const cwdAbs = path.resolve(".");
    const paths = String(envPath).split(path.delimiter);
    for (const p of paths) {
      if (!p) continue;
      const abs = path.resolve(p);
      if (abs === cwdAbs || abs.startsWith(cwdAbs + path.sep)) return false;
    }
  }
  try {
    const usePath = envPath || process.env.PATH || "";
    const out = child_process.execSync(`command -v ${bin}`, {
      encoding: "utf8",
      env: { ...process.env, PATH: usePath },
    });
...
/** 从执行选项 opts 中提取自定义 PATH（实际执行环境的命令查找路径） */
function getEnvPath(opts) {
  try {
    if (opts && typeof opts === "object" && opts.env && typeof opts.env.PATH === "string") {
...
/**
 * 检测 opts.env 是否显式注入危险环境变量（相对继承环境的增量）
 * 静默门只检查命令字符串，env 里的注入完全看不到：命令串是干净的 `git pull`，
 * 但 git/node/bash 启动时会读取下列变量执行任意代码：
 *  - GIT_SSH_COMMAND / GIT_PROXY_COMMAND：git 调用时作为 shell 命令执行
 *  - GIT_CONFIG_PARAMETERS / GIT_CONFIG_COUNT(+KEY/VALUE)：git 配置注入（可放行 ext:: 任意协议）
 *  - GIT_ALLOW_PROTOCOL：放宽协议白名单
 *  - NODE_OPTIONS：node/pnpm/npm 启动时 --require 任意 js
 *  - BASH_ENV / ENV：bash/sh 启动时 source 任意脚本
 *  - LD_PRELOAD / DYLD_INSERT_LIBRARIES：任意二进制启动时预加载任意动态库
 * 与继承环境（process.env）同值的变量不算注入（运行环境自带的不归插件管）
 * @returns {string|null} "变量名=值" 或 null（无注入）
 */
const DANGER_ENV_KEYS = new Set([
  "GIT_SSH_COMMAND", "GIT_PROXY_COMMAND", "GIT_CONFIG_PARAMETERS",
  "GIT_CONFIG_COUNT", "GIT_CONFIG_KEY_0", "GIT_CONFIG_VALUE_0",
  "GIT_ALLOW_PROTOCOL", "NODE_OPTIONS", "BASH_ENV", "ENV",
  "LD_PRELOAD", "DYLD_INSERT_LIBRARIES",
  // git 提交/拉取/交互时会调起这些变量指定的程序（可为任意命令）：
  // GIT_EDITOR/GIT_ASKPASS/SSH_ASKPASS 直接执行，EDITOR/VISUAL 为通用编辑器兜底
  "GIT_EDITOR", "GIT_ASKPASS", "SSH_ASKPASS", "EDITOR", "VISUAL",
]);

function hasDangerEnv(opts) {
  try {
    const env = opts && typeof opts === "object" && opts.env && typeof opts.env === "object" ? opts.env : null;
    if (!env) return null;
    for (const k of Object.keys(env)) {
      const ku = String(k).toUpperCase();
      if (!DANGER_ENV_KEYS.has(ku)) continue;
      const v = env[k];
      if (v === undefined || v === null || String(v) === "") continue;
      // 与继承环境同值 = 运行环境自带，非插件注入
      if (k in process.env && String(process.env[k]) === String(v)) continue;
 ...
 */
function notifyPluginCmd(cmd, method, envPath, envHit) {
  const caller = getCaller();
  // 非插件调用（本体内部命令、未知来源）不提示
  if (!caller || caller === "未知来源" || !caller.startsWith("plugins/")) return;
  // env 显式注入危险环境变量（GIT_SSH_COMMAND/NODE_OPTIONS/BASH_ENV 等）：命令串再干净也不静默，
  // 注入变量在 git/node/bash 启动时执行任意代码，静默门无法从命令字符串看到
  if (envHit) {
    pluginCmdNotified.delete(caller); // 注入提示不受节流限制，每次都报
    try {
      global.logger?.mark(`[命令保护]插件执行命令[${method}]：${caller} -> ${String(cmd).slice(0, 200)}（env 注入：${envHit}）`);
    } catch (err) {}
    notifyMaster(`⚠️ 风险提示：插件【${caller}】执行命令时注入了危险环境变量（${method}）：\n${String(cmd).slice(0, 200)}\n注入：${envHit}\n已放行执行，但该变量可在命令启动时执行任意代码，请确认插件是否可信。`);
    return;
  }
  // 更新类命令静默放行：命令本身无危害（git pull / pnpm install），且可执行文件可信（防伪装）
  if (isSilentUpdateCmd(cmd) && isTrustedBinary(cmd, envPath)) return;
  // 纯版本/环境检查命令（pnpm -v、git --version 等）静默放行，且可执行文件可信
  if (isVersionCheckCmd(cmd) && isTrustedBinary(cmd, envPath)) return;
  // 命令存在性检查（command -v git / which pnpm / type -p git 等）：
  // 纯只读查询命令路径，不执行目标，静默放行
  // 含组合符号（&&/管道/分号/重定向/命令替换/换行分隔符）仍不静默，防 command -v git && rm -rf data 绕过
  if (/^(?:command\s+-v\b|which\b|type\b)/i.test(cmd) && !/[|;&<>\$\(`\n\r]/.test(String(cmd))) return;
  // 纯只读命令（git status/diff/log 查询、fastfetch 系统信息采集）静默放行，且可执行文件可信
  if (isSilentReadCmd(cmd) && isTrustedBinary(cmd, envPath)) return;
...
function wrapExec(name) {
  ...
  child_process[name] = function (cmd, opts, cb) {
    ...
    notifyPluginCmd(cmd, name, getEnvPath(opts), hasDangerEnv(opts));
...
function wrapExecFile(name) {
  ...
  child_process[name] = function (file, args, opts, cb) {
    ...
    notifyPluginCmd(cmdStr, name, getEnvPath(opts), hasDangerEnv(opts));
    return orig.apply(this, arguments);
  };
}
...
function wrapSpawnSync() {
  ...
  child_process.spawnSync = function (file, args, opts) {
    ...
    notifyPluginCmd(cmdStr, "spawnSync", getEnvPath(opts), hasDangerEnv(opts));
    return orig.apply(this, arguments);
  };
}
...
function wrapSpawn() {
  ...
  child_process.spawn = function (file, args, opts) {
    ...
    notifyPluginCmd(cmdStr, "spawn", getEnvPath(opts), hasDangerEnv(opts));
    return orig.apply(this, arguments);
  };
}
...
function wrapUtilExec() {
  ...
  util.exec = function (cmd, opts = {}) {
    ...
    notifyPluginCmd(cmd, "Bot.exec", getEnvPath(opts), hasDangerEnv(opts));
