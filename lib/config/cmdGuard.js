...
function notifyPluginCmd(cmd, method) {
  ...
  // 更新类命令静默放行：命令本身无危害（git pull / pnpm install），任何调用者执行都不提示
  if (isSilentUpdateCmd(cmd)) return;
  // 纯版本/环境检查命令（pnpm -v、git --version 等）静默放行，命令本身无副作用
  if (isVersionCheckCmd(cmd)) return;
  // 纯只读命令（git status/diff/log 查询、fastfetch 系统信息采集）静默放行，无副作用
  if (isSilentReadCmd(cmd)) return;
