...
class Puppeteer {
  ...
  async initPupp() {
    ...
    try {
      ...
      puppeteer = null
      logger.error("puppeteer 未安装，无法使用截图功能")
    }
  ...
  async screenshot(name, data = {}) {
    ...
    let savePath = await this.dealTpl(name, data);
  ...
  async dealTpl(name, data) {
    ...
    let tmpHtml
    try {
      const _require = createRequire(_path + "/node_modules/art-template/package.json")
      const template = _require("./lib/index")
      tmpHtml = template.render(this.html[tplFile], data)
    } catch (e) {
      logger.error(`art-template 加载失败: ${e.message}`)
      return false
    }
  ...
  async watch(tplFile) {
    ...
    try {
      ...
      logger.error("chokidar 未安装，无法监听模板文件")
      return
    }
