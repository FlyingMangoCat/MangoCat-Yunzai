...
class PluginsLoader {
  ...
  filtEvent(e, v) {
    ...
    // 事件类型映射表，定义了不同类型事件的字段结构
    let eventMap = {
      message: ["post_type", "message_type", "sub_type"],  // 消息事件：上报类型、消息类型、子类型
      notice: ["post_type", "notice_type", "sub_type"],    // 通知事件：上报类型、通知类型、子类型
      request: ["post_type", "request_type", "sub_type"],  // 请求事件：上报类型、请求类型、子类型
    };
      ...
      for (let val of e.message) {
        switch (val.type) {
          case "text":
            ...
            if (
              !hasText &&
              /星铁|崩坏星穹铁道|铁道|星轨|星穹铁道|\/common\//.test(val.text)
            ) {
              e.isSr = true;
            }
            if (e.msg) {
