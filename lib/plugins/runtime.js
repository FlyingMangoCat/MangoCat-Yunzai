...
export default class Runtime {
  ...
  static async init(e) {
    ...
    let user = await MysInfo.getNoteUser(e);
    // e.user 包 Proxy：uid/uidList/mysUser/ckUidList 等快捷属性按当前游戏(e.game)自动透传
    if (user) {
      e.user = new Proxy(user, {
        get(self, key) {
          ...
          if (
            ["getUidData", "hasUid", "addRegUid", "delRegUid", "setMainUid"].includes(key)
          ) {
            return (uid, _game = "") => {
  ...
  async render(plugin, path, data = {}, cfg = {}) {
    ...
    // liulian-plugin 资源路径（布局、字体、背景等通用资源）
    let liulianResPath = `../../../${lodash.repeat("../", paths.length)}plugins/liulian-plugin/resources/`;
    // 布局文件路径（使用 liulian-plugin 的布局）
    const layoutPath = `${process.cwd()}/plugins/liulian-plugin/resources/common/layout/`;
    // 渲染data
    data = {
      sys: {
        scale: 1,
        copyright: `Created By MangoCat-Yunzai`,
      },
      /** 布局模板 **/
      defaultLayout: layoutPath + "default.html",
      elemLayout: layoutPath + "elem.html",
      /** 通用资源路径 **/
      _res_path: liulianResPath,
