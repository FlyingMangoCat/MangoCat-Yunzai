...
import "./config/fsGuard.js"
import "./config/cmdGuard.js"
...
export default class Yunzai extends EventEmitter {
  ...
  async run() {
    ...
    this.stat.online = 1
    /** 创建 Bot Proxy，使 Bot.xxx 访问走 this.bots → this → util 的优先级 */
    global.Bot = new Proxy(this, {
      get: (target, prop) => {
        // _events 直接走 target，EventEmitter 内部用私有字段操作它
        if (prop === "_events") return target._events
        // EventEmitter 核心方法绑定到 target，避免 Proxy 导致内部 Symbol/私有字段访问异常
        if (typeof target[prop] === "function" && (prop === "on" || prop === "once" || prop === "emit" || prop === "addListener" || prop === "removeListener" || prop === "removeAllListeners" || prop === "off" || prop === "listeners" || prop === "rawListeners" || prop === "listenerCount" || prop === "eventNames")) {
          return target[prop].bind(target)
        }
        const value = this.bots[prop] ?? target[prop] ?? util[prop]
        if (value !== undefined) return value
        for (const i of [target.uin.toString(), ...target.uin])
          if (this.bots[i]?.[prop] !== undefined) {
            util.makeLog("trace", `因不存在 Bot.${prop} 而重定向到 Bot.${i}.${prop}`)
            if (typeof this.bots[i][prop]?.bind === "function") return this.bots[i][prop].bind(this.bots[i])
            return this.bots[i][prop]
          }
        util.makeLog("trace", `不存在 Bot.${prop}`)
      },
      set: (target, prop, value) => {
        this.bots[prop] = value
        return true
      },
    })
    ...
    // 初始化 icqq 配置：首次启动时根据 qq.yaml 是否配置了 QQ 号自动设置
    this.initIcqqConfig()

    // 根据 icqq 配置决定是否自动登录
    const icqqCfg = cfg.getConfig("icqq")
    if (icqqCfg?.autoLogin) {
      await this.startIcqq()
    }
    ...
    util.makeLog(
      "info",
      `连接地址：${logger.blue(`${(this.url || cfg.server.url || "http://localhost").replace(/^http/, "ws")}/`)}${logger.cyan(`[${Object.keys(this.wsf)}]`)}`,
      "WebSocket",
    )
    this.emit("online", this)
  }

  /** 初始化 icqq 配置：首次启动时根据 qq.yaml 自动设置 */
  initIcqqConfig() {
    const icqqCfg = cfg.getConfig("icqq")
    // 如果 icqq 配置已启用过，跳过首次检测
    if (icqqCfg?.enable !== undefined) return
    // 首次启动：检测 qq.yaml 是否配置了 QQ 号
    const yaml = cfg.getConfig("qq")
    const enable = !!yaml?.qq
    const path = "config/config/icqq.yaml"
    const content = fsSync.readFileSync(path, "utf8")
    fsSync.writeFileSync(
      path,
      content
        .replace(/^enable:.*/m, `enable: ${enable}`)
        .replace(/^autoLogin:.*/m, `autoLogin: ${enable}`),
      "utf8",
    )
    // 清空缓存，下次读取时重新加载
    const key = "config.icqq"
    delete cfg.config[key]
    util.makeLog("info", `icqq 配置已初始化：enable=${enable} autoLogin=${enable}`, "ICQQ")
  }

  /** 启动icqq登录 */
  async startIcqq() {
    if (this.icqqClient) {
      util.makeLog("info", "icqq 已登录，无需重复启动", "ICQQ")
      return
    }
    if (!cfg.qq) {
      util.makeLog("warn", "请先在 config/config/qq.yaml 中配置QQ号", "ICQQ")
      return
    }

    util.makeLog("info", "正在启动 icqq 登录...", "ICQQ")

    const client = createClient(cfg.bot)
    this.icqqClient = client

    // 桥接 icqq 事件到 Bot
    client.on("system.login.qrcode", event => {
      this.emit("system.login.qrcode", event)
    })
    client.on("system.login.slider", event => {
      this.emit("system.login.slider", event)
    })
    client.on("system.login.device", event => {
      this.emit("system.login.device", event)
    })
    client.on("system.login.error", event => {
      this.emit("system.login.error", event)
    })

    // 登录成功后注册到 Bot
    client.on("system.online", () => {
      const uin = client.uin
      client.adapter = { id: "ICQQ", name: "icqq" }
      this.bots[uin] = client
      if (!this.uin.includes(uin)) this.uin.push(uin)
      util.makeLog("mark", `icqq(${uin}) 已登录`, "ICQQ")
      this.emit(`connect.${uin}`, { self_id: uin })
    })

    // 离线/错误事件
    client.on("system.offline", () => {
      util.makeLog("warn", "icqq 已离线", "ICQQ")
    })

    client.on("message", event => {
      event.bot = client
      // 黑名单群消息直接掐断：不进入事件分发，插件拿不到任何消息，普通群不受影响
      if (event.group_id && !cfg.checkGroup(event.group_id)) {
        util.makeLog("debug", ["黑名单群消息，已在入口拦截", event.group_id], event.self_id)
        return
      }
      this.emit("message", event)
    })

    await client.login(cfg.qq, cfg.pwd)
  }

  /** 关闭icqq登录 */
  async stopIcqq() {
    if (!this.icqqClient) {
      util.makeLog("info", "icqq 未运行", "ICQQ")
      return
    }
    const uin = this.icqqClient.uin
    this.icqqClient.emit("system.offline")
    this.icqqClient.disconnect?.()
    this.icqqClient = null
    if (uin) {
      delete this.bots[uin]
      const idx = this.uin.indexOf(uin)
      if (idx !== -1) this.uin.splice(idx, 1)
    }
    // 更新配置文件
    this.setIcqqConfig(false, false)
    util.makeLog("mark", "icqq 已关闭", "ICQQ")
  }

  /** 更新 icqq 配置文件 */
  setIcqqConfig(enable, autoLogin) {
    const path = "config/config/icqq.yaml"
    const content = fsSync.readFileSync(path, "utf8")
    fsSync.writeFileSync(
      path,
      content
        .replace(/^enable:.*/m, `enable: ${enable}`)
        .replace(/^autoLogin:.*/m, `autoLogin: ${autoLogin}`),
      "utf8",
    )
    // 清空缓存，下次读取时重新加载
    delete cfg.config["config.icqq"]
  }
  ...
  em(name = "", data = {}) {
    // 黑名单群消息直接掐断：不进入事件分发，插件拿不到任何消息
    // 仅拦截黑名单群，普通群不受影响（cfg.checkGroup 白名单优先、黑名单其次）
    if (name.startsWith("message") && data.group_id && !cfg.checkGroup(data.group_id)) {
      util.makeLog("debug", ["黑名单群消息，已在入口拦截", data.group_id], data.self_id)
      return false
    }
    if (name.startsWith("message")) {
      console.log("DEBUG em:", name, data.raw_message)
    }
  ...
  sendGroupMsg(bot_id, group_id, ...args) {
    try {
      // 兼容旧调用：sendGroupMsg(群号, 消息内容)，此时 bot_id 为群号
      if (!args.length && this.pickGroup(bot_id, true)) {
        // 主动发送出口统一校验黑白名单，防止插件绕过消息入口的黑名单检查
        if (!cfg.checkGroup(bot_id)) {
          util.makeLog("warn", ["黑名单群，拒绝发送", bot_id], `${bot_id} => ${group_id}`, true)
          return false
        }
      ...
      // 主动发送出口统一校验黑白名单，防止插件绕过消息入口的黑名单检查
      if (!cfg.checkGroup(group_id)) {
        util.makeLog("warn", ["黑名单群，拒绝发送", group_id], `${bot_id} => ${group_id}`, true)
        return false
      }
  ...
  async restart() {
    await this.redisExit()
    // execve 原地替换进程（PID 不变、接管原终端）。失败（如平台不支持/多线程限制）时
    // 不能中断重启流程，回退到下方正常重启，确保老进程可靠退出、新进程正常接管
    if (process.platform !== "win32") {
      try {
        process.execve?.(process.argv[0], process.argv, process.env)
      } catch (err) {
        Bot.makeLog("warn", `execve 重启不可用，回退常规重启：${err?.message}`, this.id)
      }
    }
    ...
    switch (global.start_type) {
      case "pm2":
        ...
        break
      case "internal":
        // 先创建新进程接管当前控制台窗口（stdio inherit、同进程组），
        // 成功后关闭 readline 恢复终端，最后退出旧进程
        util.cmdStart(process.argv[0], process.argv.slice(1))?.unref?.()
        const stdinSdk = Bot.stdin?.sdk
        if (stdinSdk) {
          stdinSdk.removeAllListeners?.("close")
          stdinSdk.close()
        }
        break
      default:
        // 前台运行（node .）时，用 spawn + stdio inherit 启动新进程接管控制台，
        // 避免 exec 管道方式导致新进程失去终端控制（不受控）
        util.cmdStart(process.argv[0], process.argv.slice(1))?.unref?.()
        break
    }
    process.exit()
