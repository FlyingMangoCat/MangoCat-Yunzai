...
export default class MysSign extends base {
  ...
  async doSign(ck, isLog = true) {
    ...
    this.mysApi = new MysApi(
      ck.uid,
      ck.ck,
      { log: isLog, device_id: ck.device_id },
      this.isSr,
    );
  ...
  async getReward(signDay) {
    ...
    if (reward) {
      ...
      if (data && data.awards && data.awards.length > 0) {
        ...
        let monthEnd =
          Number(moment().endOf("month").format("X")) -
  ...
  async bbsSign() {
    ...
    if (
      sign.retcode === 0 &&
      (sign?.data.success === 0 || sign?.message === "OK")
    ) {
  ...
  async signTask(manual) {
    ...
    logger.mark(
      `签到ck:${uids.length}个，预计需要${this.countTime(time)} ${finishTime} 完成`,
    );
  ...
  async setCache(day) {
    let end =
      Number(moment().endOf("day").format("X")) - Number(moment().format("X"));
