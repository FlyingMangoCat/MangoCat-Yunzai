...
export default class Ledger extends base {
  ...
  dealData(ledgerInfo) {
    ...
    ledgerInfo.month_data.gacha = (
      ledgerInfo.month_data.current_primogems / 160
    ).toFixed(0);
    ledgerInfo.month_data.last_gacha = (
      ledgerInfo.month_data.last_primogems / 160
    ).toFixed(0);
    if (ledgerInfo.month_data.current_primogems > 10000) {
      ledgerInfo.month_data.current_primogems =
        (ledgerInfo.month_data.current_primogems / 10000).toFixed(2) + " w";
    }
    if (ledgerInfo.month_data.last_primogems > 10000) {
      ledgerInfo.month_data.last_primogems =
        (ledgerInfo.month_data.last_primogems / 10000).toFixed(2) + " w";
    }
    if (ledgerInfo.month_data.current_mora > 10000) {
      ledgerInfo.month_data.current_mora =
        (ledgerInfo.month_data.current_mora / 10000).toFixed(1) + " w";
    }
    ...
    if (ledgerInfo.day_data.current_primogems > 10000) {
      ledgerInfo.day_data.current_primogems =
        (ledgerInfo.day_data.current_primogems / 10000).toFixed(1) + " w";
    }
    if (ledgerInfo.day_data.current_mora > 10000) {
      ledgerInfo.day_data.current_mora =
        (ledgerInfo.day_data.current_mora / 10000).toFixed(1) + " w";
    }
    ...
    for (let i in ledgerInfo.month_data.group_by) {
      ledgerInfo.month_data.group_by[i].color =
        color[ledgerInfo.month_data.group_by[i].action_id];
    }
    ledgerInfo.color = [];
    ledgerInfo.month_data.group_by.forEach((item) => {
      ledgerInfo.color.push(
        [
          "#73a9c6",
          "#d56565",
          "#70b2b4",
  ...
  async ledgerTask(manual) {
    ...
    logger.mark(
      `札记ck:${uids.length}个，预计需要${this.countTime(uids.length)} ${finishTime} 完成`,
    );
  ...
  async ledgerCountHistory() {
    ...
    if (!NoteData) {
      this.e.reply(`uid：${uid} ${nowYear}年无原石统计数据！`, false, {
        at: true,
      });
      return false;
    }
