...
export default class MysNews extends base {
  ...
  async getNews() {
    ...
    const page =
      this.e.msg.replace(/#|＃|星铁|星穹|铁道|公告|资讯|活动/g, "").trim() || 1;
    if (page > data.length) {
    ...
    return await this.replyMsg(
      img,
      `崩坏星穹铁道${typeName}：${param.data.post.subject}`,
    );
  }
  ...
  async mysNewsTask(type = 1) {
    ...
    // 包含关键字不推送
    let banWord =
      /冒险助力礼包|纪行|预下载|脚本外挂|集中反馈|已开奖|云·原神|魔神任务|传说任务说明/g;
  ...
  async sendNews(groupId, typeName, postId) {
    ...
    let tmp = await this.replyMsg(
      this[postId].img,
      `崩坏星穹铁道${typeName}推送：${this[postId].title}`,
    );
